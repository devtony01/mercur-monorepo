import ProductPageAccordion from "./product-page-accordion"
import Image from "next/image"

interface Seller {
  id?: string
  name?: string
  photo?: string
  description?: string
  rating?: number
  total_sales?: number
}

interface ProductDetailsSellerProps {
  seller?: Seller | any
}

export default function ProductDetailsSeller({ seller }: ProductDetailsSellerProps) {
  if (!seller) return null

  return (
    <ProductPageAccordion heading="Seller Information" defaultOpen={false}>
      <div className="flex items-start gap-4">
        <div className="w-16 h-16 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center flex-shrink-0">
          {seller.photo ? (
            <Image
              src={seller.photo}
              alt={seller.name || "Seller"}
              width={64}
              height={64}
              className="w-full h-full object-cover"
            />
          ) : (
            <div className="w-full h-full bg-gray-200 flex items-center justify-center text-lg font-medium text-gray-600">
              {(seller.name || "S").charAt(0).toUpperCase()}
            </div>
          )}
        </div>
        
        <div className="flex-1">
          <h4 className="font-semibold text-gray-900 mb-1">
            {seller.name || "Store"}
          </h4>
          
          {seller.rating && (
            <div className="flex items-center gap-2 mb-2">
              <div className="flex items-center">
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    className={`w-4 h-4 ${
                      i < Math.floor(seller.rating)
                        ? "text-yellow-400"
                        : "text-gray-300"
                    }`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
              <span className="text-sm text-gray-600">
                ({seller.rating.toFixed(1)})
              </span>
            </div>
          )}
          
          {seller.total_sales && (
            <p className="text-sm text-gray-600 mb-2">
              {seller.total_sales} sales
            </p>
          )}
          
          {seller.description && (
            <p className="text-sm text-gray-600">
              {seller.description}
            </p>
          )}
        </div>
      </div>
    </ProductPageAccordion>
  )
}