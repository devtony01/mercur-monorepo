...
export const loginWithGoogle = async () => {
  try {
    // Use Medusa's built-in Google OAuth flow
    const authUrl = `${process.env.MEDUSA_BACKEND_URL}/auth/customer/google`
    
    if (typeof window !== "undefined") {
      // Redirect to Medusa's Google OAuth endpoint
...
// Google OAuth callback is now handled directly in the callback page component
// using Medusa's built-in OAuth endpoints